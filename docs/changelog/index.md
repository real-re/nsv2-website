...
- ✨新特性
    ...
    - UI：角色界面双击可替换技能的技能按钮时，将更新相应技能的图标及描述`（指卡卡西）`

- 🚀变更
    - 平衡`修罗道` 阵亡后场上所有的导弹会立即爆炸`（v2.1.4 补丁）`

- ⚡️优化
    - UI：移动端 攻击按键缩放倍数支持到1.75x`（v2.1.4 补丁）`
    - UI：无忍具模式的结算界面将不再显示忍具栏位
    - 更改忍具【速走忍鞋】描述，详见[忍具说明](../wiki/items.md)

- 🔧修复
    - 🔥修复`长门`奥义2 通灵的`天道`阵亡后导致的崩溃`（v2.1.4 补丁）`
    - 🔥修复`长门`奥义2 通灵的`修罗道`阵亡后与部分角色会产生几率崩溃`（v2.1.4 补丁）`
    - 修复`飞段`奥义2 不存在诅咒仪式图案时，通过释放【忍具陷阱】即可触发反伤`（v2.1.4 补丁）`
    - 修复玩家控制的角色觉醒后，【忍术大全】减少CD的效果消失`（v2.1.4 补丁）`
...
- 🔧修复
    ...
    - 修复角色处于固定的移动状态后，使用`速走忍鞋`不会立即变更移动速度
    - 修复角色处于固定的移动状态时，`速走忍鞋`状态结束后不会立即变更移动速度
