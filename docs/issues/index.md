...
!!! bug "部分手机设备点击技能按钮需要向上一点才会响应"

    等待后续修复，#187。

!!! bug "部分手机设备屏幕过宽，屏幕上侧会出现黑边"

    等待后续修复。

!!! bug "声音输出设备变更后可能导致没有声音"

    部分平台已经实现，等待后续完善，详见路线图-其他。

!!! success "Windows平台窗口化时点击移动窗口标题栏，程序会冻结"

    遗留部分问题，点击移动时会导致一些画面问题。

## v2.1.6 已知问题

!!! success "192. 瞬身术减少技能冷却后，技能遮罩可能进度显示不正确"

!!! note "193. 移动端的设置里部分功能未完全实现"

    如切换按键缩放、布局等。

!!! bug "194. 移动端的技能二图标有一点偏向左侧"

!!! bug "195. PC端修改按键后再点击重置不会立即生效"

!!! success "196. 购买琳的立绘会吞掉金币"

    旧版本部分数据不兼容，后续会做处理。

!!! success "197. 鸣人-色诱术、井野在某些情况下可能会导致游戏崩溃"

!!! success "198. 水门技能一在某些情况下无法使用"

!!! bug "199. 阿飞奥义二自杀后，可能显示多次击杀阿飞"

!!! bug "200. PC端切换窗口大小后，当前界面布局可能会有问题"

    该功能未实现，等待后续完善。

!!! bug "201. 鹿丸的奥义一成功控制目标后，当其使用替身木后仍会受到持续伤害"

    历史遗留。

!!! success "202. 角色被攻击到地图外后将无法移动"
...
!!! success "177. 瞬身术有时会使角色始终处于无敌状态"

!!! success "178. 克隆模式，有时在加载界面同时显示6个人，导致崩溃"

    历史遗留，在游戏开始后可以点击返回按钮。

!!! success "179. 【苦无陷阱】有时击杀青蛙后没有消失"
...
!!! success "189. 宇智波鼬用奥义一击杀青蛙后不加金币"

    历史遗留，部分角色的技能可能也会有此问题。

!!! bug "190. 鸣人血条可能会消失且分身变为一丝血"

    鸣人与剧毒苦无在某些情况下会触发此bug。

!!! bug "191. 青蛙在某些情况下会处于无敌状态"
...
!!! note "156. 角都击杀一个已被夺取心脏的角色后会额外获得一个心脏"
...
!!! success "136. 鼬和佐助须佐能乎状态下，敌方角色攻击时会导致角色方向改变，且不能攻击敌方角色"
...
!!! bug "50. 很多角色技能后面都多一段硬直"
    ...
    可能是内部有些不同，导致硬直效果不正确，主要表现为更远的移动距离，还在排查中。
